...
const listGaragesValidation = [
  ...
  query('includeInactiveServices').optional().isIn(['true', 'false', '1', '0', 'yes', 'no', 'on', 'off']).withMessage('includeInactiveServices invalide'),
  query('minRating').optional().isFloat({ min: 0, max: 5 }).withMessage('minRating doit etre compris entre 0 et 5'),
  query('maxRating').optional().isFloat({ min: 0, max: 5 }).withMessage('maxRating doit etre compris entre 0 et 5'),
  query('serviceIds').optional().matches(/^\d+(\s*,\s*\d+)*$/).withMessage('serviceIds doit etre une liste d identifiants separes par des virgules'),
  query('services').optional().isString().withMessage('services doit etre une liste de noms separes par des virgules'),
  query('serviceMatch').optional().isIn(['any', 'all']).withMessage('serviceMatch invalide'),
