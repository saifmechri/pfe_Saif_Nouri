/**
 * Calcule la distance entre deux points gÃ©ographiques en kilomÃ¨tres (Haversine formula)
 ...
 * @returns {string} Distance formatÃ©e
 */
...
/**
 * Obtient la couleur basÃ©e sur la distance
 * @param {number} distance - Distance en km
...
/**
 * Obtient l'Ã©tiquette de distance
 * @param {number} distance - Distance en km
 * @returns {string} Label descriptif
 */
export const getDistanceLabel = (distance) => {
  if (distance <= 5) return "TrÃ¨s proche";
  if (distance <= 15) return "Proche";
  if (distance <= 30) return "Ã€ proximitÃ©";
  return "Loin";
};
